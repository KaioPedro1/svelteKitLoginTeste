<script>
  ...
  import ModalCardDetalhe from "./modalCardDetalhe.svelte/BtnModal.svelte";
...
<Card padding="sm" style={"margin-left:5px; margin-top:5px"}>
  ...
  <div class="flex flex-col items-center pb-4">
    ...
    <div class="flex mt-4 space-x-3 lg:mt-6">
      ...
      {#if popupModalJitsi}
        <ModalJitsi sessao_id = {sessao.id} bind:popupModalJitsi={popupModalJitsi}/>
      {/if}
      <Button
        color="light"
        class="dark:text-white"
        on:click={() => (popupModalDetalhes = true)}>Detalhes</Button
      >{#if popupModalDetalhes}
      <ModalCardDetalhe {anime} {sessao} {auditorio} bind:popupModalDetalhes={popupModalDetalhes}/>
      {/if}
    </div>
  </div>
</Card>
